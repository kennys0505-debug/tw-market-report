...
class PipelineTests(unittest.TestCase):
    ...
    def test_fixture_build_is_self_contained(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(len(payload["module_calculation_notes"]), 7)
            self.assertTrue(all("模組合成" in note for note in payload["module_calculation_notes"].values()))
            self.assertIn("今日計算", html)
