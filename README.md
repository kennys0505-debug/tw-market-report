...
- 外資期貨採方案七：部位歷史水位40%、5日變化30%、占全市場OI 20%、方向持續性10%；不再使用不具規模意義的跨商品淨部位一致比。
...
- 活躍計分採 `free_official_proxy_v2`：只納入每日可由免費官方來源穩定取得的指標。頁面分開顯示當日實測覆蓋率與20日歷史校準成熟度。
- 免費代理不冒充原指標：單日指數／廣度不稱20／60日均線，借券餘額不稱借券賣出餘額，融資金額／成交值不稱整戶維持率。
